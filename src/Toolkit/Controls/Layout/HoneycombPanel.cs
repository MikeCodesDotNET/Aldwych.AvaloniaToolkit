...
namespace Aldwych.AvaloniaToolkit.Controls
{
    public class HoneycombPanel : Panel
    {
        ...
        public static readonly StyledProperty<double> SpacingProperty = AvaloniaProperty.Register<HoneycombPanel, double>(nameof(Spacing), 0.0);
        ...
        static HoneycombPanel()
        {
            AffectsMeasure<HoneycombPanel>(SpacingProperty);
        }
        ...
        protected override Size MeasureOverride(Size availableSize)
        {
            ...
            foreach (Control child in Children)
            {
                ...
                {
                    if (Spacing != 0)
                        child.Margin = new Thickness(Spacing);

                    child.Measure(availableSize);
                    var maxWidth = Math.Max(maxSize.Width, child.DesiredSize.Width);
                    var maxHeight = Math.Max(maxSize.Height, child.DesiredSize.Height);
        ...
        private class HoneycombStuffer
        {
            ...
            public Rect Move()
            {
                ...
                if (_currentIndex > _maxIndex)
                {
                    ...
                    _currentIndex = 0;
                    _childBounds = _childBounds.Offset(_offsetX, -_offsetY);
                ...
                if (_turns > 0)
                {
                    ...
                    _childBounds = _childBounds.Offset(_offsetXArr[index], _offsetYArr[index]);
                }
